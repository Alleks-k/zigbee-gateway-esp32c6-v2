...
esp_err_t api_usecase_wifi_save(api_usecases_handle_t handle, const api_wifi_save_request_t *in)
{
    ...
    esp_err_t err = api_usecases_require_wifi_system(handle);
    if (err != ESP_OK) {
        return err;
    }
...
esp_err_t api_usecase_factory_reset(api_usecases_handle_t handle)
{
    ...
    ret = api_usecases_require_wifi_system(handle);
    if (ret != ESP_OK) {
        return ret;
    }
...
{
    esp_err_t ret = api_usecases_require_wifi_system(handle);
    if (ret != ESP_OK) {
        return ret;
    }
...
esp_err_t api_usecase_jobs_submit(api_usecases_handle_t handle, gateway_core_job_type_t type, uint32_t reboot_delay_ms,
                                  uint32_t *out_job_id)
{
    esp_err_t ret = api_usecases_require_jobs(handle);
    if (ret != ESP_OK) {
        return ret;
    }
