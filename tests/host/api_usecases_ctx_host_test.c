...
typedef void (*sig_api_usecases_set_ws_providers_t)(api_usecases_handle_t, api_ws_client_count_provider_t,
                                                     api_ws_metrics_provider_t, api_ws_provider_ctx_t *);
...
_Static_assert(__builtin_types_compatible_p(__typeof__(((api_usecases_init_params_t *)0)->ws_provider_ctx),
                                             api_ws_provider_ctx_t *),
               "api_usecases_init_params_t.ws_provider_ctx must be typed");

static uint32_t count_provider(api_ws_provider_ctx_t *ctx)
{
    return (ctx != NULL) ? *(const uint32_t *)(const void *)ctx : 0u;
}

static bool metrics_provider(api_ws_provider_ctx_t *ctx, api_ws_runtime_metrics_t *out_metrics)
{
    if (!ctx || !out_metrics) {
        return false;
    }

    out_metrics->connections_total = *(const uint32_t *)(const void *)ctx;
...
int main(void)
{
    ...
    api_usecases_init_params_t params = {
        ...
        .ws_provider_ctx = (api_ws_provider_ctx_t *)&ws_count,
    };
