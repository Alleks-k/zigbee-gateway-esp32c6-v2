 ...
 * New code should include/use zigbee_service.h, wifi_service.h, system_service.h directly.
 */

esp_err_t zgw_service_get_network_status(zgw_network_status_t *out)
{
    return zigbee_service_get_network_status(out);
}

esp_err_t zgw_service_permit_join(uint16_t seconds)
{
    return zigbee_service_permit_join(seconds);
}
...
{
    return zigbee_service_get_devices_snapshot(out, max_items);
}
